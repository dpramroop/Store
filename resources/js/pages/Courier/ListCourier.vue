<template>
    <article class="item-card" @keyup.enter="toggle" tabindex="0" role="button" :aria-expanded="open.toString()">
        <header class="item-header" @click="toggle">
            <h3 class="item-name">{{ courier.fname }} {{ courier.lname }}</h3>
            ...
            <UpdateCourier :courier_given="courier" v-on:courier-updated="notifyUpdate"/>
        </header>

        <transition name="fade">
  <div v-if="open" class="details">
    <div class="field"><strong>Email:</strong> <span>{{ courier.email || '' }}</span></div>
  <div class="field"><strong>Contact Number:</strong> <span>{{ courier.contact_no || '' }}</span></div>
...
const emits = defineEmits<{
    (e: 'courier-updated', courier: any): void
}>()
...
function notifyUpdate(updatedCourier: any) {
    emits('courier-updated', updatedCourier)
}
</script>
